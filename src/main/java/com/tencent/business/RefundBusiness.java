...
public class RefundBusiness {
    ...
    public void run(RefundReqData refundReqData,ResultListener resultListener) throws Exception {
        ...
        refundServiceResponseString = new HttpsRequest("refund").sendPost(Configure.REFUND_API, refundReqData);


        long costTimeEnd = System.currentTimeMillis();
        long totalTimeCost = costTimeEnd - costTimeStart;
        Util.debug("api请求总耗时："+ totalTimeCost + "ms");
        Util.debug("退款查询API返回的数据如下：");
        Util.debug(refundServiceResponseString);
        ...
        if (refundResData == null || refundResData.getReturn_code() == null) {
        	Util.debug("退款API请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问");
