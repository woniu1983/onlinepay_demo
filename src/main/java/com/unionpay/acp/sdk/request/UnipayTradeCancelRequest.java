 ...
 * @since JDK 1.6 
 */
public class UnipayTradeCancelRequest extends AbstractUnipayRequest {
...
//	private String merId;
	...
	/** 
	 * 字段： orderId
	 * 说明：消费撤销订单号 （商户端生成表示申请二维码和一笔真实订单的订单号）
	 * 默认：
	 * 要求： Must */
	...
	public UnipayTradeCancelRequest() {
		super();
		this.txnType = "31";
		this.txnSubType = "00";
...
//	}
	...
	public static UnipayTradeCancelRequest build(UniPayOrder order) {
		UnipayTradeCancelRequest request = new UnipayTradeCancelRequest();
		
		String txnTime = createCurrentTime();
		request.setTxnTime(txnTime);
		
		String cancelOrderId = generateCancelOrderId();
		request.setOrderId(cancelOrderId);
	...
	private static String generateCancelOrderId() {
		
		long time = System.currentTimeMillis();   // 目前13位
		long random = (long)(Math.random() * 100000L); // 最大6位
		String no = "UNPC" + random + time; // GW+ 机器
		Logger.Debug("generateCancelOrderId = " + no);
		return no;
	}
	...
	public Map<String, String> buildRequestData() {
		...
		contentData.put("txnType", 		this.txnType);      //交易类型 31
